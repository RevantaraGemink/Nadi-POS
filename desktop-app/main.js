...
app.whenReady().then(() => {
  ...
  if (!fs.existsSync(dbPath) || dbIsBroken) {
    if (fs.existsSync(oldDbPath)) {
      ...
      const templateDbPath = path.join(__dirname, 'server', 'template.db');
      if (fs.existsSync(templateDbPath)) {
        try {
          fs.copyFileSync(templateDbPath, dbPath);
          console.log('Database baru berhasil dibuat dari template!');
        } catch (e) {
