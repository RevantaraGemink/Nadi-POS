...
const NavLink = ({ href, icon, label, isActive }) => {
  return (
    <Link href={href} className={isActive ? "bg-primary-container text-on-primary-container font-bold rounded-lg flex items-center gap-3 p-3 transition-transform hover:opacity-90" : "text-on-surface-variant hover:bg-surface-container-high transition-all flex items-center gap-3 p-3 rounded-lg"}>
      ...
      <span className="font-label-caps text-label-caps hidden xl:inline">{label}</span>
    </Link>
  );
};
...
export default function Layout({ children }) {
  ...
  return (
    <>
      <nav className="bg-surface border-r border-outline-variant docked left-0 h-full w-16 xl:w-64 flex flex-col gap-unit p-2 xl:p-4 shrink-0 z-20 print:hidden transition-all duration-200">
        <div className="flex items-center gap-3 px-0 xl:px-2 mb-4 xl:mb-8 mt-2 justify-center xl:justify-start">
          <img src="/logo.jpg" alt="Logo Nadi" className="h-10 w-10 xl:h-20 xl:w-20 rounded-full object-cover shadow-sm" />
          <div className="font-headline-md text-headline-md font-bold text-primary tracking-tight hidden xl:block">Nadi</div>
        </div>
